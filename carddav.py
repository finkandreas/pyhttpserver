...
class CardDav(object):
  def __init__(self):
    self.db = DAL('sqlite://carddav.db', folder=BaseDirectory.save_data_path('pyhttpserver'))
  ...
  def sync_local_changes(self, davStorageUrl):
    ...
    for newItem in self.db((self.db.colitems.local_status == 3) & (self.db.colitems.collection == collectionId)).select():
      print("Adding a new vcard to the server")
